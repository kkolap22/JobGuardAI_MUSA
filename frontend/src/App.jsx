...
function Header() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[#e4e7ec]/80 bg-white/85 backdrop-blur-xl">
      <div className="flex h-[76px] w-full items-center justify-between px-6 lg:px-12">
        <Link to="/" className="font-display text-2xl font-bold tracking-tight">
          Job<span className={green}>Guard+</span>
        </Link>
        <nav className="hidden h-full items-center gap-6 md:flex">
          {links.map(([to, label]) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `group relative flex h-[76px] min-w-[70px] items-center justify-center px-2 text-sm font-bold whitespace-nowrap transition-colors ${isActive ? green : "text-slate-600 hover:text-[#08ad50]"}`
              }
            >
              {({ isActive }) => (
                <>
                  <span>{label}</span>
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-12 overflow-hidden">
                    <span
                      className={`block h-[2.5px] w-full rounded-full bg-[#08ad50] transition-transform duration-300 origin-right ${isActive ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
                    />
                  </div>
                </>
              )}
        ...
        <div className="hidden items-center gap-4 md:flex">
          {user ? (
            <>
              <span className="flex items-center gap-2 text-sm font-bold text-[#07843e]">
                <UserRound size={17} />
              ...
              <button
                ...
                className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-bold text-red-600 shadow-sm transition hover:border-red-600 hover:bg-red-600 hover:text-white"
              >
                Logout
              </button>
            </>
            ...
            <>
              <Link
                to="/login"
                className="rounded-lg border border-[#e4e7ec] px-4 py-2 text-sm font-bold text-slate-700 hover:border-[#08ad50] hover:text-[#08ad50]"
              >
                Login
              </Link>
        ...
        <div className="border-t border-slate-100 bg-white px-5 py-4 md:hidden">
          {links.map(([to, label]) => (
            <NavLink
              ...
              to={to}
              className="block border-b border-slate-100 py-3 text-sm font-bold text-slate-700"
            >
              {label}
            </NavLink>
          ))}
          <Link
            to={user ? "/scan" : "/login"}
            onClick={() => setOpen(false)}
            className="mt-4 block text-sm font-bold text-[#08ad50]"
          >
            {user ? "Open workspace" : "Login / Sign up"}
          </Link>
        </div>
      )}
    </header>
...
function Footer() {
  return (
    <footer className="flex flex-col justify-between gap-2 border-t border-slate-200 px-5 py-7 text-xs text-slate-400 sm:flex-row lg:px-10">
      <span>JobGuard+ © 2026</span>
      <span>Safer Jobs. Smarter Decisions.</span>
    </footer>
  );
}
...
function Tag({ icon: Icon = ShieldCheck, children }) {
  return (
      ...
      <section className="hero-grid border-b border-slate-100/80">
        <div className="w-full items-center justify-between gap-8 px-6 py-16 lg:grid lg:min-h-[640px] lg:grid-cols-[0.9fr_1.2fr] lg:px-12 lg:py-10">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-7 font-display text-6xl font-bold leading-[.94] tracking-[-.06em] sm:text-7xl lg:text-[82px]">
              Find Jobs,
              <br />
...
function SectionIntro({ tag, title, text }) {
  return (
...
function Cta() {
  return (
    <section className="mx-5 mb-16 flex flex-col justify-between gap-6 rounded-3xl bg-[linear-gradient(120deg,#067b3b,#0cbd5b)] p-8 text-white sm:p-12 lg:mx-10 lg:flex-row lg:items-center">
      ...
      <Link
        to="/scan"
        className="inline-flex w-fit items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-[#08783b]"
      >
...
function AuthPage({ mode }) {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      if (isLogin) await api.login(form.email, form.password);
      else await api.register(form.name, form.email, form.password);
      navigate("/scan");
    } catch (err) {
  ...
  return (
    <div className="auth-bg flex min-h-[calc(100vh-153px)] items-center justify-center px-5 py-16">
      <form
        onSubmit={submit}
        className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-2xl shadow-slate-900/10 sm:p-10"
      >
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#eafff1] text-[#08ad50]">
          <Icon size={28} />
        </div>
        <div className="mt-6 text-center">
          <Tag>{isLogin ? "WELCOME BACK" : "JOIN JOBGUARD+"}</Tag>
          <h1 className="mt-4 font-display text-3xl font-bold">
            {isLogin ? "Sign in to your account" : "Create your account"}
          </h1>
          <p className="mt-2 text-sm text-slate-500">
            {isLogin
              ? "Continue checking jobs with confidence."
              : "Start making safer career decisions."}
          </p>
        </div>
        ...
        <button disabled={loading} className={`${button} mt-6 w-full`}>
          {loading ? (
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          {isLogin ? "New to JobGuard+? " : "Already have an account? "}
          <Link
            className="font-bold text-[#08ad50]"
...
function Scan() {
  ...
  const loadResult = async (id) => {
    const [s, r, f] = await Promise.all([
      api.getScan(id),
      api.getReport(id).catch(() => ({ data: null })),
      api.getScanFindings(id).catch(() => ({ data: [] })),
    ]);
    setScan(s.data);
    setReport(r.data);
    setFindings(f.data || []);
  ...
  useEffect(() => {
    ...
    if (id) loadResult(id).catch((e) => setError(e.message));
  }, [params]);
  const submit = async (e) => {
    e.preventDefault();
    setError("");
    ...
    try {
      ...
      for (let attempt = 0; attempt < 60 && !done; attempt++) {
        ...
        const current = await api.getScan(id);
        setScan(current.data);
        done = ["COMPLETED", "FAILED", "STOPPED"].includes(current.data.status);
      }
      if (!done)
        throw new Error("Scan timed out. History se result check karein.");
      await loadResult(id);
      setProgress(100);
    } catch (err) {
      setError(err.message);
  ...
  const score = scan?.riskScore ?? report?.riskScore ?? 0;
  const level = (
    scan?.riskLevel ||
    report?.riskLevel ||
    "EVALUATING"
    ...
    <Layout>
      <section className="mx-auto max-w-4xl px-5 py-16 text-center">
        ...
        <h1 className="mt-6 font-display text-5xl font-bold tracking-tight sm:text-7xl">
          Check a Job
          <br />
          <span className={green}>Before You Apply.</span>
        </h1>
        <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-500">
          Paste a job posting URL and JobGuard+ will investigate its behaviour,
          redirects and suspicious signals.
        </p>
      </section>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Enter the job posting URL below to begin.
              </p>
            </div>
          </div>
          <label className="mt-8 block text-sm font-bold">
            ...
            <div className="mt-2 flex h-14 items-center gap-3 rounded-xl border border-slate-200 px-4 focus-within:border-[#08ad50] focus-within:ring-4 focus-within:ring-[#08ad50]/10">
              <Link2 className="text-slate-400" size={19} />
              <input
                value={url}
          ...
          >
            {loading ? (
              ...
              <>
                <Radar size={19} />
                Scan Job <ArrowRight size={18} />
              </>
            )}
          </button>
          {loading && (
            <div className="mt-5 h-2 overflow-hidden rounded-full bg-[#dff3e6]">
              <div
                className="h-full rounded-full bg-[#08ad50] transition-all"
                style={{ width: `${progress}%` }}
              />
            </div>
          )}
          <p className="mt-5 flex items-center justify-center gap-2 text-xs text-slate-500">
            <LockKeyhole size={14} /> Securely processed in an isolated sandbox.
          </p>
        </form>
        {scan && (
          <Result
    ...
    <article className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-xl sm:p-9">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 break-all text-xs text-slate-500">{scan.jobUrl}</p>
        </div>
        <div
          className={`grid h-12 w-12 place-items-center rounded-xl ${failed || risk === "danger" ? "bg-red-50 text-red-600" : risk === "warn" ? "bg-amber-50 text-amber-600" : "bg-[#eafff1] text-[#08ad50]"}`}
        >
          {failed || risk === "danger" ? <TriangleAlert /> : <ShieldCheck />}
        </div>
      </div>
      <div
        className={`mt-7 flex items-center gap-6 rounded-2xl p-6 ${failed || risk === "danger" ? "bg-red-50" : risk === "warn" ? "bg-amber-50" : "bg-[#f4fff8]"}`}
      >
        <strong
          className={`font-display text-6xl ${failed || risk === "danger" ? "text-red-600" : risk === "warn" ? "text-amber-600" : "text-[#08ad50]"}`}
        >
          {failed ? "—" : score}
        </strong>
        <div>
          <b className="text-sm">{failed ? "SCAN FAILED" : `${level} RISK`}</b>
          <p className="mt-1 text-sm text-slate-500">
            {failed
              ? scan.error || "The scanner could not complete this job check."
              : report?.summary ||
      ...
      <div className="mt-8">
        <h3 className="font-display text-xl font-bold">
          Findings & Detected Signals
        </h3>
        {all.length ? (
          all.map((finding, i) => (
            <div
              key={i}
              className="flex gap-3 border-b border-slate-100 py-4 text-sm"
            >
          ...
          <div className="mt-4 flex items-center gap-2 text-sm text-red-700">
            ...
            No safety conclusion was produced because the scan failed.
          </div>
        ) : (
          <div className="mt-4 flex items-center gap-2 text-sm text-[#07843e]">
            <CheckCircle2 size={18} />
            No high-risk scam signals detected.
          </div>
        )}
      </div>
    </article>
...
function History() {
  ...
  const filtered = useMemo(
    () =>
      scans.filter((s) =>
        s.jobUrl?.toLowerCase().includes(query.toLowerCase()),
      ),
    [scans, query],
  );
  return (
    <Layout>
      <section className="hero-grid px-5 py-16 text-center lg:px-10">
        <Tag icon={HistoryIcon}>YOUR SCAN ACTIVITY</Tag>
        ...
        <p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-slate-500">
          Review detected risks and revisit your previous job investigations
      ...
      <section className="mx-auto max-w-6xl px-5 pb-20 lg:px-10">
        <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          ...
          <span className="inline-flex w-fit items-center gap-2 rounded-xl bg-[#eafff1] px-3 py-2 text-xs font-bold text-[#07843e]">
            <HistoryIcon size={15} />
        ...
        <div className="space-y-3">
          {filtered.map((scan) => (
            <HistoryRow key={scan.scanId} scan={scan} />
          ))}
        </div>
        {!filtered.length && !error && (
          <div className="rounded-2xl border border-dashed border-slate-300 p-12 text-center">
            <HistoryIcon className="mx-auto text-[#08ad50]" size={36} />
            <h3 className="mt-4 font-display text-xl font-bold">
              No scan records found
            </h3>
            <Link to="/scan" className={`${button} mt-5`}>
              Start a Job Scan <ArrowRight size={17} />
            </Link>
          </div>
        )}
...
function HistoryRow({ scan }) {
  const level = (
    scan.riskLevel ||
    (scan.riskScore > 65 ? "HIGH" : scan.riskScore > 30 ? "MEDIUM" : "LOW")
  ).toUpperCase();
  ...
  return (
    <article className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg md:grid-cols-[1.5fr_.8fr_.65fr_auto] md:items-center">
      <div className="flex min-w-0 items-center gap-3">
        <div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#eafff1] text-[#08ad50]">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-slate-500">{scan.jobUrl}</p>
        </div>
      </div>
      <div className="text-xs text-slate-500">
        <span className="block uppercase tracking-wider">Scanned</span>
      ...
      <span
        className={`w-fit rounded-full px-3 py-2 text-xs font-bold ${danger ? "bg-red-50 text-red-700" : warn ? "bg-amber-50 text-amber-700" : "bg-[#eafff1] text-[#08783b]"}`}
      >
        {danger
          ? "High Risk"
          : warn
