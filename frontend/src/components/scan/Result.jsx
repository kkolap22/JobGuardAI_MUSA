...
export default function Result({ scan, report, findings = [], risk, score, level }) {
  ...
  return (
    <article className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-xl sm:p-9">
      ...
      <div className="mt-8"><h3 className="font-display text-xl font-bold">Findings &amp; Detected Signals</h3>{all.length ? all.map((finding, index) => <div key={index} className="flex gap-3 border-b border-slate-100 py-4 text-sm"><CircleAlert className="shrink-0 text-amber-500" size={18} /><span><b>{formatFindingType(finding.type)}: </b>{finding.evidence || finding.explanation || finding.description || "Signal detected"}</span></div>) : <div className="mt-4 flex items-center gap-2 text-sm text-[#07843e]"><CheckCircle2 size={18} />No high-risk scam signals detected.</div>}</div>
    </article>
  );
}